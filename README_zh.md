...
LangTARS 是基于 **Nanobot 内核**开发的 **LangBot 原生插件**，旨在为 LangBot 用户带来 **OpenClaw** 般的体验。它使您能够通过 IM 消息使用自主 AI 任务规划来控制您的 Mac。
...
| 选项 | 描述 | 默认值 |
|------|------|--------|
| `allowed_users` | 允许控制此 Mac 的用户 ID | [] |
...
```
IM 消息 --> LangBot --> PlannerTool (ReAct 循环) --> 工具 --> Mac 操作
```
